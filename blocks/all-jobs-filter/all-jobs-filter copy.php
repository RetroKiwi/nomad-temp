...
$job_location_key = 'jobcategory';
$job_location_display_name = 'Location';
$job_location = get_terms(array(
    'taxonomy' => $job_location_key,
    'hide_empty' => false,
));

$job_languages_key = 'jobslanguage';
$job_languages_display_name = 'Languages';
$job_languages = get_terms(array(
    'taxonomy' => $job_languages_key,
    'hide_empty' => false,
));

$job_type_key = 'jobstype';
$job_type_display_name = 'Types of Work';
$job_type = get_terms(array(
    'taxonomy' => $job_type_key,
    'hide_empty' => false,
));
?>
    ...
    <div class="section-header width-full m-x-auto" style="padding: 5rem var(--padding-page)">
        <div class="width-full flex-col flex-align-center flex-align-center p-x-page">
            <div class="flex-row flex-center" style="margin-bottom: 1rem; width: 100%">
                <h4><?php echo $job_location_display_name ?></h4>
            </div>
			<div class="custom-select">
                <select class="taxonomy1-select" name="taxonomy1-select">
                    <?php foreach ($job_location as $term) : ?>
            ...
            <dialog id="filterDialog" class="filter-dialog">
                ...
                <div class="dialog-content">
                        ...
                        <fieldset class="contents">
                            <?php foreach ($job_languages as $term) : ?>
                                <label class="amenities-filter-option">
                                    <i class="fa-regular fa-square"></i>
                                    <i class="fa-solid fa-square-check"></i>
                                    <input type="checkbox" class="taxonomy2-checkbox" name="taxonomy2-checkbox" value="<?php echo $term->slug; ?>">
                                    <?php echo $term->name; ?>
                                </label>
                                <br>
                            <?php endforeach; ?>
                    ...
                    <div class="flex-row flex-center" style="margin-bottom: 1rem; width: 100%; border-bottom: solid 2px var(--gray)">
                        <h4><?php echo $job_type_display_name ?></h4>
                    </div>
                    <div class="width-full flex-row flex-center flex-wrap" style="gap: 2px; margin-bottom: 3rem">
                        <fieldset class="contents">
                            <?php foreach ($job_type as $term) : ?>
                                <label class="amenities-filter-option">
                                    <i class="fa-regular fa-square"></i>
                                    <i class="fa-solid fa-square-check"></i>
                                    <input type="checkbox" class="taxonomy3-checkbox" name="taxonomy3-checkbox" value="<?php echo $term->slug; ?>">
                                    <?php echo $term->name; ?>
                                </label>
                                <br>
                            <?php endforeach; ?>
...
jQuery(document).ready(function($) {
    ...
    $('.taxonomy1-checkbox, .taxonomy2-checkbox, .taxonomy3-checkbox').change(function() {
        let taxonomy1 = '<?php echo $job_languages_key ?>';
        let taxonomy2 = '<?php echo $job_location_key ?>';
        let taxonomy3 = '<?php echo $job_type_key ?>'; // Adjust with your new taxonomy key
        let terms1 = [];
        let terms2 = [];
        let terms3 = [];
        ...
        $('.taxonomy3-checkbox:checked').each(function() {
            terms3.push($(this).val());
        });
