		...
		<section class="flex-col flex-align-center bg-black p-x-page p-y-page relative">
			...
			<div class="flex-col flex-align-center relative width-full">
				...
				<div class="flex-row flex-align-start flex-center text-light-gray" style="padding: 3rem 0; gap: 1rem">
					...
					<small class="uppercase transition-all text-hover-red bg-hover-light-gray" style="padding: .5rem 1rem; cursor: pointer" title="Location"><button style="padding: 0 .5rem"><i class="fa-solid fa-map-location-dot"></i></button><span><?php echo get_term( $select_location_type )->name; ?></span></small>
					<?php endif; ?>
					<?php if ($job_languages):?>
					<small class="uppercase transition-all text-hover-red bg-hover-light-gray" style="padding: .5rem 1rem;" title="Languages"><button style="padding: 0 .5rem"><i class="fa-solid fa-comment"></i></button><span><?php foreach ($job_languages as $index => $language): ?>
    <?php echo get_term($language)->name; ?>
    <?php if ($index < count($job_languages) - 1): ?>
        , 
    <?php endif; ?>
<?php endforeach; ?></span></small>
					<?php endif; ?>
					<?php if ($job_title):?>
					<small class="uppercase transition-all text-hover-red bg-hover-light-gray" style="padding: .5rem 1rem;" title="Job Position"><button style="padding: 0 .5rem"><i class="fa-solid fa-briefcase"></i></button><span><?php echo $job_title;?></span></small>
					<?php endif; ?>
				</div>
